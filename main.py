...
def create_post(args):
    session = bsky_login_session(args.get('pds_url'), args.get('handle'), args.get('password'))
    ...
    if args.get('lang'):
        post["langs"] = args.get('lang')

    # parse out mentions and URLs as "facets"
    if len(args.get('text')) > 0:
        facets = parse_facets(args.get('pds_url'), post["text"])
        if facets:
            post["facets"] = facets

    # if this is a reply, get references to the parent and root
    if args.get('reply_to'):
        post["reply"] = get_reply_refs(args.get('pds_url'), args.get('reply_to'))

    if args.get('image'):
        post["embed"] = upload_images(
            args.get('pds_url'), session["accessJwt"], args.get('image'), args.get('alt_text')
        )
    elif args.get('embed_url'):
        post["embed"] = fetch_embed_url_card(
            args.get('pds_url'), session["accessJwt"], args.get('embed_url')
        )
    elif args.get('embed_ref'):
        post["embed"] = get_embed_ref(args.get('pds_url'), args.get('embed_ref'))

    print("creating post:", file=sys.stderr)
    print(json.dumps(post, indent=2), file=sys.stderr)

    resp = requests.post(
        args.get('pds_url') + "/xrpc/com.atproto.repo.createRecord",
